				...
				plugin, new Runnable() {
					...
					public void run() {
						if (taskState.equals(TaskState.Running)) {
							PluginTimeChangeEvent event = new PluginTimeChangeEvent(
									plugin, timer.getTime() + 1);
							...
							for (TribePlayer player : TribeCraft
									.getPlayerManager().getPlayers()) {
								for (int i = 0; i < player.getDelayTaskList()
										.size(); i++) {
									DelayTask task = player.getDelayTaskList()
											.get(i);
