				...
				if (args[0].equalsIgnoreCase("survival")) {
					for (DelayTask task : tribePlayer.getDelayTaskList()) {
						if (task.getDescription() != null) {
							if (task.getDescription().equalsIgnoreCase(
									"Teleport")) {
					...
					final Area area = new Area(TribeCraft
							.getResidenceManager()
							.getByName(
									TribeCraft.getPluginManager()
											.getSurvivalProperties().mainTown)
							.getArea(
									TribeCraft.getPluginManager()
											.getSurvivalProperties().subArea)
							.getHighLoc(), TribeCraft
							.getResidenceManager()
							.getByName(
									TribeCraft.getPluginManager()
											.getSurvivalProperties().mainTown)
					...
					if ((player.getWorld()
							...
							&& (Area.isAreaContainLocation(area,
									player.getLocation()))) {
						getServer().getScheduler().runTask(TribeCraft.plugin,
								new Runnable() {
									@Override
									public void run() {
										while (true) {
											...
											int x = random
													.nextInt(TribeCraft
															.getPluginManager()
															.getSurvivalProperties().maxX) + 1;
											int z = random
													.nextInt(TribeCraft
															.getPluginManager()
															.getSurvivalProperties().maxZ) + 1;
											int h = player.getWorld()
													.getMaxHeight();
											...
											for (int y = h; y > 0; y--) {
												...
												if (!(block.getType()
													...
													if ((!block
														...
														Block block1 = location
																.getBlock();

														if (!Area
																.isAreaContainLocation(
																		area,
																		location)) {
															if (TribeCraft
																	.getResidenceManager()
