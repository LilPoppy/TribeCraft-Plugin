...
public class Listeners implements Listener {
	...
	public void onPlayerDeath(PlayerDeathEvent event) {
		...
		if (player.getCraftPlayer().getInventory().getHelmet() != null) {
			...
			map.put(ArmorType.Helmet, player.getCraftPlayer().getInventory()
					.getHelmet());
			equiements.put(player.getCraftPlayer().getInventory().getHelmet(),
					map);
		}
		if (player.getCraftPlayer().getInventory().getChestplate() != null) {
			HashMap<ArmorType, ItemStack> map = new HashMap<ArmorType, ItemStack>();
			map.put(ArmorType.Chestplate, player.getCraftPlayer()
					.getInventory().getChestplate());
			equiements.put(player.getCraftPlayer().getInventory()
					.getChestplate(), map);
		}
		if (player.getCraftPlayer().getInventory().getLeggings() != null) {
			HashMap<ArmorType, ItemStack> map = new HashMap<ArmorType, ItemStack>();
			map.put(ArmorType.Leggings, player.getCraftPlayer().getInventory()
					.getLeggings());
			equiements.put(
					player.getCraftPlayer().getInventory().getLeggings(), map);
		}
		if (player.getCraftPlayer().getInventory().getBoots() != null) {
			HashMap<ArmorType, ItemStack> map = new HashMap<ArmorType, ItemStack>();
			map.put(ArmorType.Boots, player.getCraftPlayer().getInventory()
					.getBoots());
			equiements.put(player.getCraftPlayer().getInventory().getBoots(),
					map);
		}

		if (!player.getCraftPlayer().hasPermission(
				new Permissions().deathSaveAll)) {
			...
			if (player.getVIPList().size() > 0) {
				...
				player.getCraftPlayer().sendMessage(
						"物品掉落机率: "
								+ (player.getVIPList().get(0)
										.getItemDropChance() * 100) + "%");
				player.getCraftPlayer().sendMessage(
						"装备掉落机率: "
								+ (player.getVIPList().get(0)
										.getArmorDropChance() * 100) + "%");
				player.getCraftPlayer().sendMessage(
						"经验掉落百分比: "
								+ (player.getVIPList().get(0)
										.getExpDropPercentage() * 100) + "%");

				player.setDeathProtectedExp((int) (player.getCraftPlayer()
						.getTotalExperience() * player.getVIPList().get(0)
						.getExpDropPercentage()));
				event.setDroppedExp((int) (player.getCraftPlayer()
						.getTotalExperience() * player.getVIPList().get(0)
						.getExpDropPercentage()));
				...
				player.getCraftPlayer()
						.sendMessage(
								"物品掉落机率: "
										+ (TribeCraft.config
												.getDouble("全局配置.死亡保护.普通用户.物品掉落机率") * 100)
										+ "%");
				player.getCraftPlayer()
						.sendMessage(
								"装备掉落机率: "
										+ (TribeCraft.config
												.getDouble("全局配置.死亡保护.普通用户.装备掉落机率") * 100)
										+ "%");
				player.getCraftPlayer()
						.sendMessage(
								"经验掉落百分比: "
										+ (TribeCraft.config
												.getDouble("全局配置.死亡保护.普通用户.经验掉落百分比") * 100)
										+ "%");

				player.setDeathProtectedExp((int) (player.getCraftPlayer()
						.getTotalExperience() * (TribeCraft.config
						.getDouble("全局配置.死亡保护.普通用户.经验掉落百分比"))));
				event.setDroppedExp((int) (player.getCraftPlayer()
						.getTotalExperience() * (TribeCraft.config
						.getDouble("全局配置.死亡保护.普通用户.经验掉落百分比"))));
				...
				for (ItemStack item : items.keySet()) {
					...
					for (int i = 0; i <= item.getAmount(); i++) {
						if (Math.random() <= TribeCraft.config
								.getDouble("全局配置.死亡保护.普通用户.物品掉落机率")) {
							newItem.setAmount(newItem.getAmount() - 1);
						}
					}
				...
				for (HashMap<ArmorType, ItemStack> map : equiements.values()) {
					for (ArmorType type : map.keySet()) {
						...
						if (Math.random() > TribeCraft.config
								.getDouble("全局配置.死亡保护.普通用户.装备掉落机率")) {
							equiements.put(armor, null);
							inventory.equiments.put(type, armor);
						}
					}
	...
	public void onPlayerRespawn(PlayerRespawnEvent event) {
		...
		PlayerStoreExperienceEvent event2 = new PlayerStoreExperienceEvent(
				player);
			...
			player.addDelayTask(new DelayTask(1, "Experience") {
				...
				public void run() {
					if (!player.getCraftPlayer().isDead()
							&& player.getCraftPlayer().isOnline()) {
						ISystem.experience.setTotalExperience(
								player.getCraftPlayer(),
								player.getDeathProtectedExp());
					}
				}
			});
		}
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		if (!TribeCraft.getPlayerManager().hasPlayer(
				event.getPlayer().getName())) {
			final TribePlayer player = new TribePlayer(event.getPlayer());
			TribeCraft.getPlayerManager().setPlayer(
			...
			if (event.getPlayer().getItemInHand().getType()
				...
				if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
					if (event.getClickedBlock().getType() == Material
							.getMaterial(6)) {
						PlayerIncSackEvent event1 = new PlayerIncSackEvent(
								TribeCraft.getPlayerManager().getPlayer(
										event.getPlayer().getName()));
						getServer().getPluginManager().callEvent(event1);
						...
						if (event1.isCancelled()) {
							...
							event.getPlayer().sendMessage(
									ChatColor.RED + "本服务器禁止使用骨粉对 "
											+ ChatColor.WHITE
											+ Material.getMaterial(6).name()
											+ ChatColor.RED + " 进行催长!");
						}
					}
				}
	...
	public void onPlayerPortal(final PlayerPortalEvent event) {
		if (event.getCause().equals(TeleportCause.NETHER_PORTAL)) {
			...
			if (TribeCraft.getPortalGateManager()
				...
				PlayerUseGateEvent event1 = new PlayerUseGateEvent(TribeCraft
						.getPlayerManager().getPlayer(
								event.getPlayer().getName()), gate);
				getServer().getPluginManager().callEvent(event1);
				...
				if (gate.getType().equals(PortalGateType.Location)) {
					...
					TribeCraft.getPlayerManager()
							.getPlayer(event.getPlayer().getName())
							.addDelayTask(new DelayTask(5) {
								@Override
								public void run() {
									TribeCraft.getPermissionManager()
											.playerRemove(event.getPlayer(),
													"Tribe.user.survival");
								}
							});
			...
			if (task.getDescription() != null) {
				if (task.getDescription().equalsIgnoreCase("Teleport")) {
					if ((event.getFrom().getX() != event.getTo().getX())
						...
						PlayerTeleportingMoveEvent event1 = new PlayerTeleportingMoveEvent(
								player);
						getServer().getPluginManager().callEvent(event1);
